...
public class FlowLayout extends ViewGroup {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (currentWidthUsed + measuredWidth > widthSize) { //剩余空间不足以显示当前child 换行
                currentWidthUsed = getPaddingLeft() + getPaddingRight() +measuredWidth;
                lastIndexEveryLine.add(i - 1);
            } else {
                currentWidthUsed += measuredWidth;
            }
        ...
        totalHeight += lineMargin*lastIndexEveryLine.size();
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            currentLeftToStart = right;
            if(lastIndexEveryLine.contains(i)){
                currentLeftToStart = getPaddingLeft();
                currentTop += child.getMeasuredHeight()+lineMargin;
            }
